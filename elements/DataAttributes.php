...
class DataAttributes extends \Widget
{
	/**
	 * Submit user input
	 * @var boolean
	...
	public function generate()
	{
		...
		/*
      <th width="15%">'.$GLOBALS['TL_LANG']['MSC']['ow_value'].'</th>
      <th width="75%">'.$GLOBALS['TL_LANG']['MSC']['ow_label'].'</th>
     */

		$return = '<table class="tl_optionwizard" id="ctrl_'.$this->strId.'" width="100%">
  <thead>
    <tr>
      <th width="15%">data-</th>
      <th width="75%">Eigenschaft</th>
      <th>&nbsp;</th>
    </tr>
  </thead>
  <tbody class="sortable" data-tabindex="'.$tabindex.'">';
    ...
    <tr>
      ...
      <td><input type="text" name="'.$this->strId.'['.$i.'][label]" id="'.$this->strId.'_label_'.$i.'" class="tl_text_2" tabindex="'.$tabindex++.'" value="'.specialchars($this->varValue[$i]['label']).'" style="width:95%;"></td>';
      ...
      <td style="white-space:nowrap; padding-left:3px">';
			...
			{
				$class = ($button == 'up' || $button == 'down') ? ' class="button-move button_'.$button.'"' : 'class="button_'.$button.'"';
